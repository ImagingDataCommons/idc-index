...
@click.argument(
    "generic_argument",
    type=str,
    help="String parameter that can contain IDC file manifest, or one or a comma separated list of collection_id, PatientID, StudyInstanceUID, or SeriesInstanceUID values. If a list of items is passed, they should all be of the same type (i.e., you cannot mix collection_id and PatientID).",
)
def download(
    generic_argument,
):
    ...
    # this is not a file manifest
        ...
        if client.index["collection_id"].str.contains(item_ids[0]):
            logger_cli.debug("Downloading from collection_id")
            client.download_from_selection(".", collection_id=item_ids)
        elif client.index["PatientID"].str.contains(item_ids[0]):
            logger_cli.debug("Downloading from PatientID")
            client.download_from_selection(".", patientId=item_ids)
        elif client.index["StudyInstanceUID"].str.contains(item_ids[0]):
            logger_cli.debug("Downloading from StudyInstanceUID")
            client.download_from_selection(".", studyInstanceUID=item_ids)
        elif client.index["SeriesInstanceUID"].str.contains(item_ids[0]):
            logger_cli.debug("Downloading from SeriesInstanceUID")
            client.download_from_selection(".", seriesInstanceUID=item_ids)
