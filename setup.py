...
class PostInstallCommand(install):
    ...
    def run(self):
        ...
        urllib.request.urlretrieve(
            ...
            + package_version
            + "/idc_index.csv.zip",
            os.path.join(save_location, "idc_index.csv.zip"),
        )
        ...
        if system == "Windows":
            urls.append(
                f"https://github.com/peak/s5cmd/releases/download/v{s5cmd_version}/s5cmd_{s5cmd_version}_Windows-64bit.zip"
            )
            s5cmd_path = os.path.join(save_location, "s5cmd.exe")
        elif system == "Darwin":
            urls.append(
                f"https://github.com/peak/s5cmd/releases/download/v{s5cmd_version}/s5cmd_{s5cmd_version}_macOS-64bit.tar.gz"
            )
            s5cmd_path = os.path.join(save_location, "s5cmd")
        else:
            urls.append(
                f"https://github.com/peak/s5cmd/releases/download/v{s5cmd_version}/s5cmd_{s5cmd_version}_Linux-64bit.tar.gz"
            )
            s5cmd_path = os.path.join(save_location, "s5cmd")
        ...
        for url in urls:
            ...
            print(
                f"s5cmd successfully downloaded and extracted, and is located at {s5cmd_path}"
            )
            os.remove(filepath)
